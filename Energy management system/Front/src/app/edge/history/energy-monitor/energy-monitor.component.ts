...
export class EnergyMonitorComponent implements OnInit {
  ...
  private token =
    'GE3GjZRFyPVkZmXHPUnSut6x2B2OpzNrMbkzXNQ1CguaKFEys43utSHEdUhB7tS15EUMEKuAbXoVlIts1bdVeg==';
  private url = 'http://localhost:8086';
  private org = 'Dev'; // You'll also need to specify the organization
  ...
  |> filter(fn: (r) => r["_field"] == "_sum/ConsumptionActivePower" or r["_field"] == "_sum/GridActivePower" or r["_field"] == "_sum/EssSoc" or r["_field"] == "_sum/ProductionAcActivePower")
  |> filter(fn: (r) => r["edge"] == "${this.selectedEdge.code}")
  |> yield(name: "mean")
    `;
